...
from network import FullyConvNetwork_facades
...
parser.add_argument('--batch_size', type=int, default=30, help='batch size, set to 1 on PC')
...
def train_one_epoch(model, dataloader, optimizer, criterion, device, epoch, num_epochs):
    ...
    for i, (image_rgb, image_semantic) in enumerate(dataloader):
        ...
        if epoch % 5 == 0 and i == 0:
            save_images(image_rgb, image_semantic, outputs, f'{args.output_folder}/train_results', epoch, args.batch_size)
...
def validate(model, dataloader, criterion, device, epoch, num_epochs):
    ...
    with torch.no_grad():
        for i, (image_rgb, image_semantic) in enumerate(dataloader):
            ...
            # Save sample images every 5 epochs
            if epoch % 5 == 0 and i == 0:
                save_images(image_rgb, image_semantic, outputs, f'{args.output_folder}/val_results', epoch, args.batch_size)
...
if __name__ == '__main__':
    ...
    model = FullyConvNetwork_facades(3).to(device)
    criterion = nn.L1Loss()
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 20 == 0:
            ...
            torch.save(model.state_dict(), f'{args.output_folder}/checkpoints/pix2pix_model_epoch_{epoch + 1}.pth')

    os.makedirs(f'{args.output_folder}/checkpoints', exist_ok=True)
    torch.save(model.state_dict(), f'{args.output_folder}/checkpoints/Pix2pix_model_final.pth')
